...
class RecommendationPipeline:
    ...
    def recommend_song(self, request: str) -> tuple[Song, str, int]:
        previous_songs = '\n'.join([
        ...
        try:
            ...
            classify_song_prompt = Prompt('classify_speed', data={
                'CURRENT_SONG_DESCRIPTION': self.get_current_song_desc(),
            })
            class_response = self.llm.request('You are an AI Music DJ', classify_song_prompt.get_content(), output_schema=SpeedRating)
            try:
                rating = min(5, max(0, int(class_response['rating'])))
            except:
                rating = 3

            return self.current_song, best_song.response, rating
    ...
    def process_request(self, request: str) -> list[dict]:
        ...
        for action in actions:
            ...
            if action_id == 1:
                if self.current_song and self.current_song.artist:
            ...
            #     self.remove_song_from_playlist(action.data)
                ...
                new_song, dj_response, rating = self.recommend_song(action.user_request)
                print(new_song.stringify_current())
                packets.extend([
                    {
                        'type': 'change_song',
                        'content': {
                            'song': new_song.model_dump(),
                            'speed_rating': rating,
                        },
                    },
                    {
